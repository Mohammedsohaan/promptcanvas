...
export function buildUserStoriesPrompt(input: PromptInput | PromptContext): string {
  if ("context" in input) {
    const { context } = input;
...
Document Title: ${input.documentTitle || "User Stories"}

Current Document Context:
${input.documentContent || "None provided."}

Additional Instructions:
${input.additionalInstructions || "Include Given-When-Then acceptance criteria for each story."}
