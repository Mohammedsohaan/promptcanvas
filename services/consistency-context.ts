  ...
  public static compute(index: ProjectIndex, graph: DocumentGraph): ConsistencyContext {
    ...
    for (const item of index.documents) {
      ...
      if (item.parentDocumentId) {
        ...
        if (item.type !== DocumentType.PRD && item.type !== "PRD" && item.type !== DocumentType.CUSTOM) {
          orphanDocuments.push({
            id: item.id,
            title: item.title,
            parentId: null,
      ...
      if (item.type === DocumentType.PRD || item.type === "PRD") {
      ...
      if (item.type === DocumentType.USER_STORIES || item.type === "USER_STORIES") {
        const children = graph.getChildren(item.id);
      ...
      if (item.type === DocumentType.API_SPEC || item.type === "API_SPEC") {
        const children = graph.getChildren(item.id);
